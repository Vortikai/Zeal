...
- `/spelleffects`, `/spellfx`
  - **Arguments:** `nosprites`, `bard <0, 1, 2, 3>`, `classic`, `default`, `<Target Spell Name or ID> classic`, `<Target Spell Name or ID> default`, `buff classic`, `buff default`, `heal classic`, `heal default`, `replace <Target Spell ID> <Source Spell ID>`, `replace <Target Spell ID> default`, `reset`
  ...
  - **Example:** `/spelleffects bard`: Sets the effects mode (0 = default, 1, 2, 3 = alternatives) of 14 bard songs
    (mostly resists and mana regen) to optionally be more subtle (0 is invisible with /showspelleffects off). Not affected by replace changes.
  ...
  - **Example:** `/spelleffects default`: Reverts all spell effects to the modern ("new) style used by default by the client. Level milestone particle effects for most spells are locked at level 24 regardless of character level.
  - **Example:** `/spelleffects <Target Spell Name or ID> classic`: Changes the effects of the specified spell to the classic style with working particle level milestones at levels 1, 24, and 39.
  - **Example:** `/spelleffects <Target Spell Name or ID> default`: Reverts the effects of the specified spell to the default style used by the client.
  - **Example:** `/spelleffects buff classic`: Changes spell effects only for the effects commonly associated with the shielding buff category (e.g. Minor Shielding, Shield of the Magi, Dead Man Floating, Rune III, etc.) to the classic ("old") style with working particle level milestones at levels 1 (green particles), 24 (green + orange), and 39 (green + orange + blue).
  - **Example:** `/spelleffects buff default`: Reverts spell effects only for the effects commonly associated with the shielding buff category (e.g. Minor Shielding, Shield of the Magi, Dead Man Floating, Rune III, etc.) to the default ("new") style. Level milestone particle effects for all buffs in this category will remain at level 24 regardless of level (green + orange).
  - **Example:** `/spelleffects heal classic`: Changes spell effects only for the effects commonly associated with the healing and regen category (e.g. Minor Healing, Complete Healing, Chloroplast, etc.) to the classic ("old") style with working particle level milestones at levels 1 (blue particles from the hands), 24 (hands + ring of particles around caster), and 39 (hands + ring + sphere around caster).
  - **Example:** `/spelleffects heal default`: Reverts spell effects only for the effects commonly associated with the healing and regen category (e.g. Minor Healing, Complete Healing, Chloroplast, etc.) to the default ("new") style. Level milestone particle effects for all heals in this category will remain at level 1 regardless of level (simple blue particles from the hand).
  ...
  - **Example:** `/spelleffects replace <Target Spell Name or ID> default`: Reverts the changes made for the target spell to the default effects used by the client.
